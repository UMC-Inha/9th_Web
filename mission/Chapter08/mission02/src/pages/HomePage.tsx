...
const HomePage = () => {
  ...
  const {
    ...
    isLoading,
    isError,
  } = useGetInfiniteLpList(20, value, order);

  const { ref, inView } = useInView({
    threshold: 0,
  });

  const throttleValue = useThrottle(inView, 3000);

  useEffect(() => {
    if (throttleValue && hasNextPage && !isFetching) {
      fetchNextPage();
    }
  }, [throttleValue, hasNextPage, fetchNextPage, isFetching]);
